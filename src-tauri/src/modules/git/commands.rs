...
pub async fn git_list_stash(
    ...
    let workspace = WorkspaceEnv::from_option(workspace);
    blocking(app, move |r| {
        operations::list_stash(r, &repo_root, &workspace).map_err(Into::into)
    })
    .await
}

#[tauri::command]
pub async fn git_push_stash(
    repo_root: String,
...
pub async fn git_pop_stash(
    ...
    let workspace = WorkspaceEnv::from_option(workspace);
    blocking(app, move |r| {
        operations::pop_stash(r, &repo_root, index, &workspace).map_err(Into::into)
    })
    .await
}
...
pub async fn git_drop_stash(
    ...
    let workspace = WorkspaceEnv::from_option(workspace);
    blocking(app, move |r| {
        operations::drop_stash(r, &repo_root, index, &workspace).map_err(Into::into)
    })
    .await
}
