...
pub fn create_branch(
    ...
    if let Some(sp) = start_point.filter(|s| !s.is_empty()) {
        if !branch_name_is_safe(sp) {
            return Err(GitError::command("git checkout -b", "invalid start point"));
        }
        sp_owned = sp.to_string();
        args.push(OsStr::new(&sp_owned));
    }
    let output = run_git(
        &repo_root.workspace,
        Some(&repo_root.git_path),
...
fn branch_name_is_safe(name: &str) -> bool {
    ...
    // Reject anything that looks like a flag or path traversal.
...
pub fn list_stash(
    ...
    let repo_root = authorized_repo_root(registry, repo_root, workspace)?;
    ensure_git_available(&repo_root.workspace)?;
    // Format: <index>\x1f<sha>\x1f<message>
    let lines = git_stdout_lines(
        &repo_root.workspace,
        &repo_root.git_path,
...
pub fn push_stash(
    ...
    let msg_owned = message.filter(|s| !s.is_empty()).map(|m| m.to_string());
    if let Some(ref m) = msg_owned {
        args.push("-m");
        args.push(m.as_str());
    }
    let output = run_git(
