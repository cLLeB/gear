        ...
        match (a.get(ai).copied(), b.get(bi).copied()) {
            ...
            (Some(ac), Some(bc)) => {
                if ac.is_ascii_digit() && bc.is_ascii_digit() {
                    ...
                    // Same value, different zero padding: order by padding so the
                    // comparator stays a total order (never Equal for distinct names).
...
/// Numeric-aware, case-insensitive name ordering, so `9.txt` precedes `10.txt`.
/// Only non-ASCII names pay for a full Unicode case fold.
