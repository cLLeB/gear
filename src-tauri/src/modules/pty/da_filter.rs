                ...
                State::InsideCsi => {
                    ...
                    if (0x40..=0x7e).contains(&b) {
                        if b == FINAL_C {
                            ...
                            // Only a DA arriving before *any* output is a genuine
                            // startup probe we should answer on the shell's behalf.
                            // Once the shell has emitted something, a DA belongs to
                            // an app that will read the real terminal's reply, so
                            // synthesizing one here would inject an out-of-order
