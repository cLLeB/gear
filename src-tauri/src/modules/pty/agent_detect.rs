...
mod tests {
    ...
    #[test]
    fn gear_marker_drives_status() {
        let mut d = AgentDetector::new();
        run(&mut d, &osc("133;C;claude"));
        assert_eq!(run(&mut d, &osc("777;notify;Gear;attention")), vec![Transition::Attention]);
        assert_eq!(run(&mut d, &osc("777;notify;Gear;working")), vec![Transition::Working]);
        assert!(run(&mut d, &osc("777;notify;Gear;working")).is_empty());
        assert_eq!(run(&mut d, &osc("777;notify;Gear;finished")), vec![Transition::Finished]);
    }

    #[test]
    fn gear_marker_auto_arms_without_preexec() {
        let mut d = AgentDetector::new();
        assert_eq!(
            run(&mut d, &osc("777;notify;Gear;attention")),
