...
# A/B (the `mark-prompt` feature); Gear disables it at spawn via
# fish_features=no-mark-prompt so these markers aren't emitted twice.

# Installed into conf.d, which every fish session sources; only Gear-spawned
# shells (GEAR_TERMINAL=1) may get their prompt wrapped.
if not set -q GEAR_TERMINAL
    exit 0
end
if set -q __GEAR_HOOKS_LOADED
    exit 0
end
...
set -g __GEAR_HOST (uname -n 2>/dev/null; or echo localhost)

# URL-encode a path keeping `/` intact so it stays valid inside file://.
function __gear_urlencode_path
...
function __gear_restore_status
    return $argv[1]
end

function __gear_capture_user_prompt
    if not functions -q fish_prompt
        return
    end
    if functions fish_prompt | string match -q '*__gear_user_prompt*'
        return
    end
    functions -e __gear_user_prompt 2>/dev/null
    functions -c fish_prompt __gear_user_prompt
end

# Wrapped so `fish -C __gear_install_prompt` can re-run it AFTER config.fish,
...
function __gear_install_prompt
    ...
    function fish_prompt
        set -l __gear_status $status
        printf '\e]133;D;%d\e\\' $__gear_status
        printf '\e]7;file://%s%s\e\\' "$__GEAR_HOST" (__gear_urlencode_path "$PWD")
        printf '\e]133;A\e\\'
        ...
        __gear_restore_status $__gear_status
        if functions -q __gear_user_prompt
            __gear_user_prompt
        else
            printf '%s > ' (prompt_pwd)
        end
...
__gear_install_prompt

function __gear_preexec --on-event fish_preexec
    set -g __gear_block_seen 1
