...
fn read_histories() -> Vec<(String, i64)> {
    ...
    // PowerShell (PSReadLine) history is a plain one-command-per-line file, so
    // the bash parser handles it. Without this, ArrowUp on Windows would only
    // ever surface commands typed in the current session.
    #[cfg(windows)]
    if let Some(path) = psreadline_histfile() {
        if let Ok(content) = std::fs::read_to_string(&path) {
            all.extend(parse_bash(&content));
        }
    }
...
#[cfg(windows)]
fn psreadline_histfile() -> Option<PathBuf> {
    dirs::data_dir()
        .map(|d| d.join("Microsoft/Windows/PowerShell/PSReadLine/ConsoleHost_history.txt"))
        .filter(|p| p.exists())
}
