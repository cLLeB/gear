...
fn run(args: Vec<OsString>) -> Result<(), CliError> {
    ...
    match config.action {
        ...
        Action::Version => {
            println!("gear {}", env!("CARGO_PKG_VERSION"));
            Ok(())
        }
        Action::Request { method, params } => {
            let endpoint = load_endpoint()?;
            let caller = env::var("GEAR_PANE_ID")
                .ok()
            ...
            if !response.ok {
                let error = response.error.unwrap_or_else(|| {
                    gear_control_protocol::ControlError::new(
                        "request_failed",
                        "Gear rejected the request",
                    )
                });
...
fn parse_open(args: Vec<OsString>) -> Result<Action, CliError> {
    ...
    let path = canonical.into_os_string().into_string().map_err(|_| {
        CliError::new(
            "non_utf8_path",
            "Gear cannot open a path that is not valid UTF-8",
            EXIT_USAGE,
        )
    })?;
...
fn load_endpoint() -> Result<ControlDescriptor, CliError> {
    let env_address = env::var("GEAR_CONTROL_ADDR").ok();
    let env_token = env::var("GEAR_CONTROL_TOKEN").ok();
    let (descriptor, require_live_process) = match (env_address, env_token) {
        ...
        (None, None) => {
            let path = dirs::cache_dir()
                .map(|dir| dir.join("gear").join("control.json"))
                .ok_or_else(|| {
                    CliError::new(
                        "app_unavailable",
            ...
            let bytes = std::fs::read(&path).map_err(|_| {
                CliError::new(
                    "app_unavailable",
                    "Gear is not running; start the app and try again",
                    EXIT_UNAVAILABLE,
                )
            })?;
            let descriptor = serde_json::from_slice(&bytes).map_err(|error| {
                CliError::new(
                    "invalid_descriptor",
                    format!("invalid Gear control descriptor: {error}"),
                    EXIT_PROTOCOL,
                )
            })?;
            (descriptor, true)
        ...
        _ => {
            return Err(CliError::new(
                "invalid_environment",
                "GEAR_CONTROL_ADDR and GEAR_CONTROL_TOKEN must be set together",
                EXIT_PROTOCOL,
            ));
        }
    };
...
fn validate_endpoint(
    ...
    if descriptor.protocol != PROTOCOL_VERSION {
        return Err(CliError::new(
            ...
            format!(
                "Gear uses control protocol {}, but this CLI supports {PROTOCOL_VERSION}",
                descriptor.protocol
            ),
            EXIT_PROTOCOL,
        ));
    ...
    if require_live_process && !process_is_alive(descriptor.pid) {
        return Err(CliError::new(
            "invalid_endpoint",
            "Gear control process is not running",
            EXIT_PROTOCOL,
        ));
    }
    Ok(descriptor)
...
fn send_request(address: &str, request: &ControlRequest) -> Result<ControlResponse, CliError> {
    ...
    let mut stream = TcpStream::connect_timeout(&address, CONNECT_TIMEOUT).map_err(|error| {
        CliError::new(
            "app_unavailable",
            format!("could not connect to Gear: {error}"),
            EXIT_UNAVAILABLE,
        )
    })?;
...
fn parse_loopback_address(address: &str) -> Result<SocketAddr, CliError> {
    let address: SocketAddr = address.parse().map_err(|error| {
        CliError::new(
            "invalid_endpoint",
            format!("invalid Gear control address: {error}"),
            EXIT_PROTOCOL,
        )
    })?;
    if !address.ip().is_loopback() {
        return Err(CliError::new(
            "invalid_endpoint",
            "Gear control address must be loopback-only",
            EXIT_PROTOCOL,
        ));
    }
    Ok(address)
...
fn read_response(
    ...
    if bytes.len() > MAX_MESSAGE_BYTES {
        return Err(CliError::new(
            "message_too_large",
            "Gear response exceeded the protocol limit",
            EXIT_PROTOCOL,
        ));
    }
    if bytes.last() != Some(&b'\n') {
        return Err(CliError::new(
            "invalid_response",
            "Gear returned an incomplete response",
            EXIT_PROTOCOL,
        ));
    }
    let response: ControlResponse = serde_json::from_slice(&bytes).map_err(|error| {
        CliError::new(
            "invalid_response",
            format!("Gear returned invalid JSON: {error}"),
            EXIT_PROTOCOL,
        )
    })?;
    let matched_id =
    ...
    if response.protocol != PROTOCOL_VERSION || !matched_id {
        return Err(CliError::new(
            "invalid_response",
            "Gear returned a mismatched protocol version or request id",
            EXIT_PROTOCOL,
        ));
    }
    Ok(response)
    ...
    match method {
        METHOD_PING => {
            let version = result
                ...
                .unwrap_or("unknown");
            println!("Gear {version} is running");
        }
        METHOD_CAPABILITIES => {
        ...
        METHOD_OPEN => {
            ...
            if let Some(line) = line {
                println!("Opened {path}:{line} in Gear");
            } else {
                println!("Opened {path} in Gear");
            }
        }
        _ => println!("{result}"),
    }
...
Usage:\n  gear <file> [--line <n>] [--no-focus] [--json]\n  gear open <file> [--line <n>] [--no-focus] [--json]\n  gear ping [--json]\n  gear capabilities [--json]\n  gear identify [--json]\n  gear --version\n\n\
The app must be running. Commands launched in a Gear pane target that pane's space."
    );
}
