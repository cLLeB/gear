...
// window can listen.
const PREFS_CHANGED_EVENT = "Gear://prefs-changed";
...
export async function loadPreferences(): Promise<Preferences> {
  ...
  return {
    ...
    backgroundKind: get<BackgroundKind>(KEY_BG_KIND) ?? DEFAULT_PREFERENCES.backgroundKind,
...
const IMPORTABLE_KEYS = new Set<string>(Object.keys(DEFAULT_PREFERENCES));
...
export async function importSettings(
  ...
  const defaults = DEFAULT_PREFERENCES as Record<string, unknown>;
  const tasks: Promise<void>[] = [];
  for (const key of IMPORTABLE_KEYS) {
    if (!(key in data)) continue;
    const value = data[key];
    const defaultType = typeof defaults[key];
    if (typeof value !== defaultType && !(defaultType === "object" && value === null)) continue;
    tasks.push(writePref(key, value));
  }
...
const KEYS_CHANGED_EVENT = "Gear://ai-keys-changed";
