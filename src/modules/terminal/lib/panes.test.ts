...
describe("swapLeafInDirection", () => {
  ...
  it("moves a pane's user-facing name with it", () => {
    const tree: PaneNode = {
      kind: "split",
      id: 100,
      dir: "row",
      children: [
        { kind: "leaf", id: 1, name: "build" },
        { kind: "leaf", id: 2, name: "server" },
      ],
    };
    const swapped = swapLeafInDirection(tree, 2, "left");
    if (swapped.kind === "split") {
      expect(swapped.children.map((c) => (c as { name?: string }).name)).toEqual(
        ["server", "build"],
      );
    }
  });
  ...
  it("does nothing when the active id is not in the tree", () => {
    const tree = row(1, 2);
    expect(swapLeafInDirection(tree, 99, "left")).toBe(tree);
  });
});
