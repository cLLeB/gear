...
describe("isAgentActivePty", () => {
  ...
  it("tracks a pty from its first phase until it is cleared", () => {
    expect(isAgentActivePty(4)).toBe(false);
    useAgentActivityStore.getState().setPhase(4, "working");
    expect(isAgentActivePty(4)).toBe(true);
    // idle still counts as tracked — only "exited" clears the pty.
    useAgentActivityStore.getState().setPhase(4, "idle");
    expect(isAgentActivePty(4)).toBe(true);
    useAgentActivityStore.getState().clear(4);
    expect(isAgentActivePty(4)).toBe(false);
  });
});
