...
function getRecycler(): HTMLDivElement {
  ...
  el.setAttribute("data-gear-recycler", "");
  el.style.cssText =
  ...
  if (!fast) {
    ...
    if (p.snapshot) {
      try {
        ...
        console.warn("[gear] snapshot replay failed:", e);
      }
    }
    if (p.altScreen) {
...
function serializeSlot(slot: Slot): SerializeOutput {
  ...
  try {
    ...
    console.warn("[gear] serialize failed:", e);
  }
  return {
    snapshot,
    cols: slot.term.cols,
...
function disposeSlot(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[gear] slot dispose failed:", e);
  }
  slot.host.remove();
...
function disposeSlotWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[gear-webgl] dispose failed:", e);
  }
  try {
    const r = (
