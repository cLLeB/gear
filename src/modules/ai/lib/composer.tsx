...
export function AiComposerProvider({ children }: ProviderProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("Gear:ai-attach-file", onAttach);
    return () => window.removeEventListener("Gear:ai-attach-file", onAttach);
  ...
  const attachFileByPath = async (path: string) => {
    try {
      ...
      if (result.kind === "toolarge") return;
      ...
      const imageMediaType = imageMediaTypes[ext];
      if (result.kind === "binary" && imageMediaType) {
        // Reload as data URL via a fetch on the Tauri asset protocol or
        // a dedicated base64 command if available. Fall back to blob URL
        // from a re-fetch using the convertFileSrc helper.
        try {
          ...
          // convertFileSrc failed (e.g. path not allowlisted); skip silently.
        }
        return;
      }
  ...
  const submit = () => {
    ...
    // the prompt to the GEAR.md scan template before sending.
    ...
    if (commandSource.startsWith("/") || commandSource.startsWith("#")) {
      ...
      if (outcome.kind === "send-prompt") {
        ...
        if (outcome.commandName) {
          commandMarker = `<Gear-command name="${outcome.commandName}" />`;
        }
      }
    }
