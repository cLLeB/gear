...
export function LocalAgentNotificationsBridge() {
  ...
  useEffect(() => {
    ...
    if (status === "awaiting-approval") {
      fire("attention", "Gear needs your approval", "Approve a tool to continue");
    } else if (status === "error") {
      fire("error", "Gear run failed", error ?? undefined);
    } else if (status === "idle" && isBusy(was)) {
      fire("finished", "Gear finished", "Your task is ready");
    }
  }, [status, error]);
