...
export function AiInputBar() {
  ...
  const supportsVision = useMemo(
    () =>
      (MODELS.find((m) => m.id === selectedModelId) as { tags?: string[] } | undefined)
        ...
        <AnimatePresence initial={false}>
          {showVisionWarning && (
            <motion.div
              ...
              className="flex items-center gap-1 px-1 text-[11px] text-amber-500"
            >
              <span>⚠</span>
              <span>
                The selected model doesn&apos;t support images — switch to a
          ...
          <PopoverAnchor asChild>
            <div className="flex items-start gap-2">
              <textarea
                ...
                placeholder="Ask Gear anything   -   # for snippets and commands, @ for files"
                rows={1}
                className={cn(
