export const KEYRING_SERVICE = "gear";
...
export type ModelInfo = {
  ...
  /** GPT-5 / frontier Claude reject a `temperature` param — omit it for these. */
...
/** GPT-5 and frontier Claude models reject a `temperature` sampling param;
 *  callers must omit it. Unknown/custom models default to supporting it. */
...
/** True when a model needs a reasoning-token output budget (extended thinking).
 *  Covers tagged reasoning models plus GPT-5 and gpt-oss families. */
...
export const SYSTEM_PROMPT = `You are Gear, the AI built into the Gear developer terminal. You are a hands-on engineer, not a chat bot — your job is to *do* the work, not narrate it.
...
export const SYSTEM_PROMPT_LITE = `You are Gear, the AI built into the Gear developer terminal. Each turn carries an <env> block (workspace_root, active_terminal_cwd, optional active_file) prepended to the user's message — treat as ground truth.
