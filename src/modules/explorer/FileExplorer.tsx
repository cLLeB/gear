  ...
  function FileExplorer(
    ...
    useEffect(() => {
      setSelectedPaths((prev) => {
        ...
        const next = new Set([...prev].filter((p) => entryIndexByPath.has(p)));
        return next.size === prev.size ? prev : next;
      });
      setAnchorPath((prev) =>
        prev !== null && entryIndexByPath.has(prev) ? prev : null,
      );
    }, [entryIndexByPath]);
    ...
    const handleSelectPath = useCallback(
      (path: string, event: React.MouseEvent) => {
        if (event.shiftKey && anchorPath) {
          const anchorIdx = entryPaths.indexOf(anchorPath);
          const targetIdx = entryPaths.indexOf(path);
    ...
    useImperativeHandle(
      ...
      () => ({
        focus: () => {
          ...
          if (anchorPath === null && entryPaths.length > 0) {
    ...
    // Determine the best parent path for a new file/folder based on selection.
    // Selected dir → create inside it. Selected file → create alongside it.
    // Nothing selected → fall back to rootPath.
    const getCreateParent = useCallback((): string => {
      if (anchorPath) {
        const idx = entryIndexByPath.get(anchorPath);
        if (idx !== undefined) {
          const row = rows[idx];
          if (row?.kind === "entry") {
            if (row.isDir) return anchorPath;
            const slash = anchorPath.lastIndexOf("/");
            if (slash > 0) return anchorPath.slice(0, slash);
          }
        }
      }
      return rootPath ?? "";
      ...
      >
        <div className="flex h-8 shrink-0 items-center gap-1 border-b border-border/60 px-2">
          ...
          <Button
            ...
            size="icon"
            className="size-6 text-muted-foreground hover:text-foreground"
            onClick={() => tree.beginCreate(getCreateParent(), "file")}
            title="New file"
          >
          ...
          <Button
            ...
            size="icon"
            className="size-6 text-muted-foreground hover:text-foreground"
            onClick={() => tree.beginCreate(getCreateParent(), "dir")}
            title="New folder"
          >
