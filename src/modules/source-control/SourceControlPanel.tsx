...
export const SourceControlPanel = memo(function SourceControlPanel({
  ...
  const loadStashes = useCallback(async () => {
    ...
    setStashError(null);
    try {
      const list = await native.gitListStash(scm.repo.repoRoot);
      setStashes(list);
    } catch (e: unknown) {
      setStashError(typeof e === "string" ? e : "Failed to load stashes");
    } finally {
  ...
  const handlePushStash = useCallback(async () => {
    if (!scm.repo) return;
    setStashBusy("push");
    setStashError(null);
    try {
      await native.gitPushStash(
        scm.repo.repoRoot,
      ...
      setStashError(typeof e === "string" ? e : "Failed to stash changes");
    } finally {
      setStashBusy(null);
    }
  }, [scm, stashMessage, loadStashes]);
  ...
  const handlePopStash = useCallback(
    async (index: number) => {
      ...
      setStashError(null);
      try {
        await native.gitPopStash(scm.repo.repoRoot, index);
        await loadStashes();
        void scm.refresh();
      } catch (e: unknown) {
        setStashError(typeof e === "string" ? e : "Failed to pop stash");
      } finally {
        setStashBusy(null);
      }
    },
  ...
  const handleDropStash = useCallback(
    async (index: number) => {
      ...
      setStashError(null);
      try {
        await native.gitDropStash(scm.repo.repoRoot, index);
        await loadStashes();
      } catch (e: unknown) {
        setStashError(typeof e === "string" ? e : "Failed to drop stash");
      } finally {
        setStashBusy(null);
      }
    },
          ...
          <>
            ...
            <div className="shrink-0 border-b border-border/40">
              <button
                ...
                className="group flex w-full cursor-pointer items-center gap-2 px-3 py-2 text-left text-muted-foreground transition-colors hover:bg-foreground/[0.04] hover:text-foreground"
              >
                <HugeiconsIcon
                  icon={Archive01Icon}
                  size={13}
                  ...
                  className="shrink-0"
                />
                <span className="flex-1 text-[12px] font-medium">Stash</span>
                <HugeiconsIcon
                  icon={stashOpen ? ArrowDown01Icon : ArrowRight01Icon}
                  size={12}
                  strokeWidth={2}
                  className="shrink-0 opacity-50 transition-transform group-hover:translate-x-0.5"
                />
              ...
              {stashOpen ? (
                <div className="space-y-2 bg-gradient-to-b from-card/65 to-card/30 px-2.5 pb-2.5">
                  <div className="flex gap-1.5">
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter" && stashBusy === null)
                          void handlePushStash();
                      }}
                      placeholder="Stash message (optional)"
                      className="min-w-0 flex-1 rounded-md border border-border/45 bg-background/90 px-2.5 py-1.5 text-[12px] outline-none placeholder:text-muted-foreground/55 focus:border-primary/45"
                    />
                    <Button
                      size="xs"
                      variant="secondary"
                      className="h-7 shrink-0 cursor-pointer text-[11.5px]"
                      disabled={stashBusy !== null}
                      onClick={() => void handlePushStash()}
                    >
                      {stashBusy === "push" ? (
                        <Spinner className="size-3" />
                      ) : (
                  ...
                  {stashLoading ? (
                    ...
                    <div className="space-y-0.5">
                      {stashes.map((s) => (
                        <div
                          key={s.index}
                          className="group/stash flex items-center gap-1.5 rounded-md px-1 py-1 hover:bg-foreground/[0.04]"
                        >
                          ...
                          <div className="flex shrink-0 items-center gap-0.5 opacity-0 transition-opacity group-hover/stash:opacity-100">
                            <IconActionButton
                              label="Pop stash"
                              disabled={stashBusy !== null}
                              side="top"
...
function BranchSwitcher({
  ...
  useEffect(() => {
    ...
    native
      ...
      .then(setBranches)
      .catch((e: unknown) =>
        setError(typeof e === "string" ? e : "Failed to load branches"),
      )
  ...
  const exactMatch = branches.includes(query.trim());
  ...
  const handleCheckout = async (b: string) => {
    ...
    setError(null);
    try {
      await native.gitCheckoutBranch(repoRoot, b);
      setOpen(false);
      onBranchChange();
    } catch (e: unknown) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setError(typeof e === "string" ? e : "Failed to create branch");
    } finally {
      setActionBusy(false);
    }
  };
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          type="button"
          disabled={disabled}
          className="inline-flex min-w-0 items-center gap-1.5 rounded-md bg-foreground/5 px-2 py-1 text-[11.5px] font-medium leading-none text-foreground transition-colors hover:bg-foreground/10 disabled:opacity-50"
        >
          <HugeiconsIcon
            icon={GitBranchIcon}
            size={12}
      ...
      >
        <div className="flex flex-col">
          <div className="border-b border-border/50 px-2.5 pt-2 pb-1.5">
            <input
              ref={inputRef}
              type="text"
              value={query}
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && canCreate && !actionBusy) {
                  void handleCreate();
                }
              }}
              placeholder="Filter or create branch…"
              className="w-full bg-transparent text-[12px] outline-none placeholder:text-muted-foreground/55"
            />
          </div>

          {error ? (
            <p className="px-3 py-1.5 text-[10.5px] text-destructive break-all">
              {error}
            </p>
          ) : null}
              ...
              <>
                {filteredBranches.map((b) => (
                  <button
                    ...
                    className={cn(
                      "flex w-full items-center gap-2 px-3 py-1.5 text-left text-[12px] hover:bg-accent transition-colors disabled:opacity-50",
                  ...
                  >
                    <span
                      className={cn(
                        "size-1.5 shrink-0 rounded-full",
                        b === branch ? "bg-emerald-500" : "bg-transparent",
                      )}
                    />
                ...
                {canCreate ? (
                  <button
                    ...
                    className="flex w-full items-center gap-2 px-3 py-1.5 text-left text-[12px] hover:bg-accent transition-colors text-muted-foreground disabled:opacity-50"
                  >
                    <span className="shrink-0 text-emerald-500 font-semibold">
                      +
                    </span>
