...
async function respond(
  ...
  if (!delivered) {
    console.warn(`[gear] control response expired: ${requestId}`);
  }
}
  ...
  useEffect(() => {
    ...
    const handleRequest = async (request: ControlRequest) => {
      try {
        ...
        if (request.method === "open") {
          ...
          if (open.focus) {
            ...
            try {
              ...
              console.warn("[gear] could not focus control target:", error);
            }
          }
          ...
          if (tabId === null) {
            throw new RequestError(
              "open_failed",
              "Gear could not create an editor tab",
            );
          }
        ...
        await respond(request.id, { ok: false, error: responseError }).catch(
          (responseError) => {
            console.error("[gear] control response failed:", responseError);
          },
        );
      }
    };

    void listen<ControlRequest>("gear:control-request", (event) => {
      ...
      .catch((error) => {
        console.error("[gear] control bridge setup failed:", error);
      });
    ...
    return () => {
      ...
      void setFrontendReady(false).catch((error) => {
        console.error("[gear] control bridge cleanup failed:", error);
      });
    };
