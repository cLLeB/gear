...
import type { GearLspClient } from "./client";
...
async function createSession(
  ...
  const [{ TauriLspTransport }, { GearLspClient }] = await Promise.all([
    import("./transport"),
    import("./client"),
  ]);

  if (GearLspClient.hostPid === null) {
    GearLspClient.hostPid = await invoke<number>("lsp_host_pid").catch(
      () => null,
    );
  }
  ...
  const client = new GearLspClient({
    transport,
    rootUri,
