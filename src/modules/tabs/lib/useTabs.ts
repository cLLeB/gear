...
export function useTabs() {
	...
	const newBlockTab = useCallback((cwd?: string, shellPath?: string) => {
		const tabId = nextIdRef.current++;
		const leafId = nextIdRef.current++;
		setTabs((t) => [
			...t,
	 ...
	 * every open tab. */
	const setLeafName = useCallback((leafId: number, name: string) => {
		setTabs((curr) =>
			curr.map((t) => {
				if (t.kind !== "terminal" || !hasLeaf(t.paneTree, leafId)) return t;
				const paneTree = setLeafNameInTree(t.paneTree, leafId, name);
