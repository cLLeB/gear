...
describe("createAgentNotificationGate", () => {
  ...
  it("keeps distinct targets and notification kinds independent", () => {
    ...
    expect(shouldDeliver({ ...TARGET, agent: "claude" }, 1_003)).toBe(true);
    expect(shouldDeliver({ ...TARGET, tabId: 8 }, 1_004)).toBe(true);
  });
  ...
  it("bounds retained notification keys", () => {
    ...
    // TARGET was evicted as the oldest key, so it is deliverable again even
    // though it is still inside the cooldown.
