...
const LOCAL_META: Partial<Record<ProviderId, LocalMeta>> = {
  lmstudio: {
    ...
    modelHint: (
      <>
        The model ID loaded in LM Studio — find it on the server's{" "}
  ...
  ollama: {
    ...
    modelHint: <>The model name shown in <code>ollama list</code> or <code>ollama pull</code>.</>,
  },
  "openai-compatible": {
        ...
        {visibleProviders.length === 0 ? (
          <div className="rounded-lg border border-dashed border-border/60 bg-card/40 px-4 py-8 text-center">
            <p className="text-[12px] text-muted-foreground">
              {t("settings.models.noProvidersTitle")}
            </p>
            <p className="mt-0.5 text-[10.5px] text-muted-foreground/70">
              {t("settings.models.noProvidersDesc")}
            </p>
          </div>
        ) : (
    ...
    <>
      ...
      {enabled && !hasKey ? (
        <p className="pl-19 text-[10.5px] text-muted-foreground">
          {t("settings.models.providerNotConnected", { provider: getProvider(provider).label })}
        </p>
      ) : null}
    </>
  );
        ...
        {setContextLimit ? (
          <FieldRow label={t("settings.models.context")}>
            <div className="flex flex-1 items-center gap-1.5">
              ...
              />
              <span className="text-[10.5px] text-muted-foreground">{t("common.tokens")}</span>
            </div>
          </FieldRow>
        ) : null}
...
function StatusLine({
  ...
  if (status === "testing") {
    return (
      <span className="text-[10.5px] text-muted-foreground">{t("settings.models.testStatus.testing")}</span>
    );
  }
  if (status === "ok") {
    return (
      <span className="flex items-center gap-1 text-[10.5px] text-muted-foreground">
        ...
        {t("settings.models.testStatus.ok")}
      </span>
    );
  }
  return (
    <span className="text-[10.5px] text-destructive/80">
      {t("settings.models.testStatus.fail")}
    </span>
  );
}
