...
| 🟡 | 42b51e7 | feat(lsp): find references picker, ruff preset | DEFER — find-references picker needs `locationsPanel.ts` (absent) AND the activation-aware multi-server selection rework (`serversForLanguage` + activation), since adding the ruff preset makes pyright/ruff both claim `.py`. Half-porting would break Python server choice. Ruff preset held until the selection contract lands. |
...
| 🟡 | a25fb40 | feat(editor): biome+prettier format-on-save | DEFER — biome/prettier format-on-save needs the external-formatter registry (`externalFormat.ts`) our fork doesn't carry; a genuine new subsystem, out of scope for a straight port. |
