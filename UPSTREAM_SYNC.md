...
| 🟡 | 89e399a | feat(agents): notification hooks for Codex/Gemini CLI | PARTIAL / DEFER (Codex+Gemini CLI hooks) — our fork already writes **Claude Code** hooks (`agent_enable_claude_hooks`, `notify;Gear;` OSC 777) and detects codex (`agent_detect` DEFAULT_AGENTS). The remaining delta is writing **Codex/Gemini** CLI hook configs (`~/.codex/hooks.json`, `~/.gemini/…`) via a per-agent spec + a self-invoked `__gear_notify` shim (those CLIs can't emit the marker themselves). Tangible reason to hold: it wires a CLI shim into app startup (lib.rs/Cargo.toml) and writes into external tools' home dirs for CLIs not installed here — unverifiable, global side effects, on a subsystem we deliberately scoped to Claude Code. |
| 🔁 | b23cd00 | feat(agents): per-agent notification bell + clear | COVERED — our fork ships a richer agent-notification system already: `NotificationBell` (per-agent bell + clear/read) and `AgentNotificationsBridge` (attention/finished routing + toasts). |
...
| 🔁 | ef55e36 | fix(agents): harden self-arm + notification UX | COVERED — attention/finished routing, toast gating, and read/clear handled by our `AgentNotificationsBridge` + `NotificationBell`. |
...
| ✅ | e874b39 | feat(lsp): cmd-hover link, hover highlight, statusbar | PORTED — spliced `hoverCodeHighlight` (tokenizes code inside LSP hover tooltips with the file's parser) + `linkHover` (cmd/ctrl-hover underlines the symbol, matching mod-click go-to-def) into `lspInteractions`; LspStatusPill border/cursor polish. Additive View plugins — no change to our diverged client core. |
...
| ✅ | 6980581 | feat(editor): find/replace panel, goto line, large file open, indent detection | PORTED (applicable parts) — **indent detection** (`indent.ts` `detectIndentUnit` + `indentCompartment`/`indentExtension`, EditorPane reconfigures per file; +6 tests). Find/replace panel + goto-line **already present** in our fork (`search({top:true})`, `searchKeymap`, `highlightSelectionMatches`). Large-file open already done in 662dbbb (`openAnyway`). formatter registry / `externalFormat.ts` since ported in **a25fb40**; `EditorSection.tsx` N/A (our settings live in GeneralSection). |
...
| ✅ | 85a5653 | fix(editor): save/reload races, formatter mtime, lsp format style + preset rebinding | PORTED (applicable parts) — writeToDisk stays dirty if edited mid-write; reload re-checks dirty after the async read; close-guard counts dirty after the busy-await; `useLspExtension` rebinds on preset swap; LSP format honors tab vs space indent (`indentUnit`). formatter-mtime / `adoptDiskText` now present via the **a25fb40** external-formatter port. |
| ✅ | 786ceb5 | feat(editor): markdown notes GFM, clickable tasks | PORTED (adapted) — markdown loader now uses `markdownLanguage` (GFM: tables/task-lists/strikethrough/autolinks) + fenced-code highlighting; `markdownExtras.ts` adds clickable `[ ]`/`[x]` task toggles and Cmd/Ctrl+click links. `markdownCodeLanguages` reads our `languageResolver` loaders (via new `codeLanguageDescriptions`) instead of upstream's absent `languageDefinitions.ts`. |
| ✅ | 7b1fae6 | feat(editor): AI autocomplete placement/quality/triggers | PORTED — our `inlineExtension.ts`/`prompt.ts` were identical to the parent so took the post-commit versions (adds `normalizeIndent`, `trimSuggestion`, `reindentClosers`, `capToLineSuffix`; +19 tests); `autocompleteTrigger` pref (auto/manual) wired through store + EditorPane getPrefs + a `Alt+\` manual-trigger keybinding + a settings toggle; autocomplete temperature 0.2→0.1. |
